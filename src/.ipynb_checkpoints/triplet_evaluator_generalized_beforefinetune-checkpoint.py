...
def evaluate_model(dataname, trainset_name, testset_name, n_data=5, num_epochs=3, base_model='SBERT'):
    ...
    for data_index in data_index_list:
        for epoch in epochs:
            ...
            train_result_name = 'roberta-base-Train_idx%d_epoch%d_before_finetune'%(data_index, epoch)
            test_result_name  = 'roberta-base-Test_idx%d_epoch%d_before_finetune'%(data_index, epoch)
            
            if base_model=='BERT': 
                train_result_name = 'bert-base-Train_idx%d_epoch%d_before_finetune'%(data_index, epoch)
                test_result_name  = 'bert-base-Test_idx%d_epoch%d_before_finetune'%(data_index, epoch)
